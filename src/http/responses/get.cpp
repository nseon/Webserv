...
{
	Request const *request = response.getRequest();

	if (request && !request->getServer()->getIndex().empty())
	{
		std::string new_path(path);
		if (new_path[new_path.size() - 1] != '/')
			new_path += '/';
		new_path += request->getServer()->getIndex();
		response.setStatusCode(200);
		response.setStatusMsg("OK");
		getFile(new_path, response);
	}
	if (request && response.getLocation().getAutoIndex() && response.getStatusCode() != 200)
	{
		std::string autoindex = generate_autoindex(path, request->getPath());
		std::stringstream ss;
